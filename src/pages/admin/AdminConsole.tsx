...
export default function AdminConsole() {
  ...
  const loadStatsAndRecent = async () => {
    ...
    try {
      ...
      const collectors =
        users.filter((u) => u.role === 'collector').length || (await count('users', [{ col: 'role', val: 'collector' }]));
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg p-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-red-100 mt-1">Super Admin: {userProfile?.full_name || 'Administrador'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded">
        <p className="text-gray-700">
          <strong>🛡️ Panel administrativo</strong>
        </p>
        <p className="text-gray-600 mt-2">
          Aquí administras usuarios y monitoreas la plataforma. (La creación/eliminación de usuarios usa Netlify Functions).
        </p>
      </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {u.is_verified ? <CheckCircle className="w-5 h-5 text-green-600" /> : <XCircle className="w-5 h-5 text-gray-400" />}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex gap-2">
                          <button
                            ...
                            title={u.is_verified ? 'Desverificar' : 'Verificar'}
                          >
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{stats.productsTotal}</span>
              </div>
              <div className="flex justify-between">
                <span>Productos activos</span>
                <span className="font-semibold">{stats.productsActive}</span>
              </div>
              <div className="flex justify-between">
                <span>Ofertas</span>
                <span className="font-semibold">{stats.offersTotal}</span>
              </div>
              <div className="flex justify-between">
                <span>Solicitudes</span>
                <span className="font-semibold">{stats.requestsTotal}</span>
              </div>
              <div className="flex justify-between">
                <span>Mensajes</span>
                <span className="font-semibold">{stats.messagesTotal}</span>
              </div>
              <div className="flex justify-between">
                <span>Favoritos</span>
                <span className="font-semibold">{stats.favoritesTotal}</span>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <li key={p.id} className="border border-gray-100 rounded-lg p-3">
                    ...
                    <div className="text-xs text-gray-600 mt-1 flex flex-wrap gap-2">
                      <span className="inline-flex items-center gap-1">
                        <Boxes className="w-3 h-3" /> {p.category}
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <MessageSquare className="w-3 h-3" /> {p.type}
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <Heart className="w-3 h-3" /> {p.status}
                      </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white w-full max-w-lg rounded-lg shadow-xl">
            ...
            <div className="p-5 space-y-4">
              ...
              {createError && (
                <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded">{createError}</div>
              )}
              <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded text-sm">
                Para crear/eliminar usuarios en Auth necesitas configurar Netlify Functions + variables de entorno.
              </div>
            </div>
